class Node:
    ...
    def __str__(self, level=0, indent="\t\t"):
        s = level * indent + str((self.key, "R" if self.color else "B"))
        if self.left:
...
class RedBlackTree:
    ...
    def __str__(self):
        return ("(tree height = {})\n".format(self.size)) + str(self.root)

    def __left_rotate(self, x):
        if not x.right:
            raise ValueError("x.right is nil!")
        y = x.right
        x.right = y.left
    ...
    def __right_rotate(self, x):
        if not x.left:
            raise ValueError("x.left is nil!")
        y = x.left
        x.left = y.right
...
# rbt.add(4)
# print(rbt)
# print(rbt.black_height())
# rbt.delete(rbt.search(3))
# print(rbt)

for i in [33, 32, 31, 100, 81, 71, 17, 16, 13, 15]:
    rbt.add(i)
print(rbt)